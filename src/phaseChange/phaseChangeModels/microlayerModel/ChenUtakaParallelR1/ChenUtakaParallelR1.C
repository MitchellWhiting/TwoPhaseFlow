...
namespace Foam
{
    ...
    defineTypeNameAndDebug(ChenUtakaParallelR1, 0);
    addToRunTimeSelectionTable(microlayerModel,ChenUtakaParallelR1, components);
    ...
    )
    {
        ...
        if (Pstream::master())
        {
            ...
            {
                Info << "ChenUtakaParallelR1 Coupling Stats: Total=" << totalFluidFaces 
                     << ", Skipped(Phase>=0.5)=" << skippedPhase 
                     << ", Skipped(dml<=1e-10)=" << skippedDml 
...
Foam::ChenUtakaParallelR1::ChenUtakaParallelR1
(
...
:
    ...
    method_(ChenUtakaCoeffs_.lookupOrDefault<string>("initialisationMethod","ChenUtakaParallelR1")), //Only working for 1 patch at the moment
...
// ************************************************************************* //
// Foam::tmp<Foam::fvScalarMatrix> Foam::ChenUtakaParallelR1::dmlInitial()
// {
//
// }


Foam::tmp<Foam::fvScalarMatrix> Foam::ChenUtakaParallelR1::hSourceML()
{
    ...
    if (fluidPatchID != -1)
    {
        ...
        {        
            ...
            {
            Pout << "ChenUtakaParallelR1::hSourceML: Local dml max=" << maxLocalDml 
...
void Foam::ChenUtakaParallelR1::initialiseML()
{
    // Support both naming conventions for compatibility
    if (method_ == "ChenUtakaParallelR1" || method_ == "chenUtaka")
    {
        Info<< "ChenUtakaParallelR1: Executing initialiseML with method " << method_ << endl;
        ...
        Info << "ChenUtakaParallelR1: Initialised dml. Local max added = " << maxInitDml << endl;
    }
    else
    {
        Info << "ChenUtakaParallelR1: Skipping initialiseML. Method '" << method_ << "' does not match 'ChenUtakaParallelR1' or 'chenUtaka'." << endl;
    }

}

void Foam::ChenUtakaParallelR1::updateML()
{
    ...
    if (debug_)
    {
        Info << "ChenUtakaParallelR1 Debug: MSource_ max = " << gMax(MSource_.internalField()) 
             << ", min = " << gMin(MSource_.internalField()) << endl;
        Info << "ChenUtakaParallelR1 Debug: dml_ max = " << gMax(dml_) 
             << ", min = " << gMin(dml_) << endl;
    }
}

Foam::tmp<Foam::volScalarField> Foam::ChenUtakaParallelR1::energySourceML()
{
...
Foam::ChenUtakaParallelR1::massSourceML( volScalarField& rhoSource)
{
...
Foam::tmp<Foam::volScalarField>
Foam::ChenUtakaParallelR1::alphaSourceML( volScalarField& rhoSource)
{
